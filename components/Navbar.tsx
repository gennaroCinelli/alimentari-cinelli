...
export default function Navbar() {
  ...
  // Controllo per sapere se siamo sulla Home Page
  ...
  const navigaVerso = (url: string) => {
    ...
    // Navigazione ISTANTANEA (Senza più il caricamento col logo che gira)
    router.push(url);
  };
  ...
  return (
    <>
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#0a0a0a]/95 backdrop-blur-xl border-b border-white/5 shadow-2xl transition-all">
        ...
        {/* PIANO INFERIORE (MENU DESKTOP) - QUESTO APPARE SOLO SE NON SIAMO NELLA HOME */}
        {!isHome && (
        ...
        {/* MENU MOBILE (Anche questo nascosto sulla Home per evitare doppioni) */}
        {!isHome && (
          <AnimatePresence>
            {isMobileMenuOpen && (
